...
class TrainSet(Dataset):
    def __init__(self, args, transform=transforms.Compose([RandomFlip(), RandomRotate(), ToTensor()]) ):
        self.input_list = sorted([os.path.join(args.dataset_dir, 'train/input_800', name) for name in 
            os.listdir( os.path.join(args.dataset_dir, 'train/input_800') )])
        self.ref_list = sorted([os.path.join(args.dataset_dir, 'train/ref_800', name) for name in 
            os.listdir( os.path.join(args.dataset_dir, 'train/ref_800') )])
